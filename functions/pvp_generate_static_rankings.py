...
if __name__ == '__main__':
    ...
    processes = []
    p = multiprocessing.Process(target=generate_lists, args=(1500, 50, cp_multiplier_50, detailed_files, "File 1",))
    processes.append(p)
    p.start()
    p = multiprocessing.Process(target=generate_lists, args=(1500, 51, cp_multiplier_51, detailed_files, "File 2",))
    processes.append(p)
    p.start()
    p = multiprocessing.Process(target=generate_lists, args=(2500, 50, cp_multiplier_50, detailed_files, "File 3",))
    processes.append(p)
    p.start()
    p = multiprocessing.Process(target=generate_lists, args=(2500, 51, cp_multiplier_51, detailed_files, "File 4",))
    processes.append(p)
    p.start()
